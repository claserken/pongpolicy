...
def make_critic_model():
    ...
    model.add(layers.Dense(10, input_shape=(80,80,1)))
...
# model = make_actor_model()

actorLocation = "./actorPPO1"
criticLocation = "./criticPPO1"

if os.path.isdir(actorLocation):
    actor_model=load_model(actorLocation)
    print('LOADED ACTOR MODEL')
else:
    actor_model = make_actor_model()
    print('MAKING NEW ACTOR MODEL')

if os.path.isdir(criticLocation):
    critic_model=load_model(criticLocation)
    print('LOADED CRITIC MODEL')
else:
    critic_model = make_critic_model()
    print('MAKING NEW CRITIC MODEL')
...
while True:
    with tf.GradientTape() as actor_tape, tf.GradientTape() as critic_tape:
        ...
        while True:
            ...
            aprob = actor_model(x.reshape(1,80,80,1), training=True)
            ...
            batched_ep_games[indexinBatch].append([cur_x, action, reward])
            ...
            if reward != 0: # Pong has either +1 or -1 reward exactly when game ends.
                # batched_ep_games[indexinBatch].append(reward)
                indexinBatch += 1
            ...
            if done: # an episode finished  
                ...
                if episode_number % batch_size == 0:
                    actor_model.save(modelLocation)
                    print('MODEL SAVED')
                env.reset()
                ...
                if episode_number % batch_size == 0:
                    for game in batched_ep_games:
                        # [cur_x, action]
                        reward_array.append(discount_rewards([k[-1] for k in game]))
                        discount_rewards(reward_array)
                        for i in range(len(discounted_rewards)):
                            weighted_log_sum += discounted_rewards[i] * game_log_array[i]      
               
                    actor_gradients = actor_tape.gradient(weighted_log_sum, actor_model.trainable_variables)
                    opt.apply_gradients(zip(actor_gradients, actor_model.trainable_variables))

                    critic_gradients = critic_tape.gradient( , critic_model.trainable_variables)
                    opt.apply_gradients(zip(critic_gradients, critic_model.trainable_variables))
